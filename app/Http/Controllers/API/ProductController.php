...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        $query = Product::query()
            ...
            ->when($request->query('sort_price'), function (Builder $query, $sort) {
                $direction = in_array($sort, ['asc', 'desc']) ? $sort : 'asc';
                $query->orderBy('price', $direction);
            });
        $perPage = $request->query('per_page', 10); // Số bản ghi trên mỗi trang (mặc định là 10)
